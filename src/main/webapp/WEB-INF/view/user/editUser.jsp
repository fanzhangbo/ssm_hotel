...
<div class="layui-form layuimini-form">
    <div class="layui-form-item">
        ...
        <div class="layui-input-block">
            <input type="text" name="username" lay-verify="required" lay-reqtext="姓名不能为空" placeholder="请输入姓名" value="${user.username}" class="layui-input">
            <tip>姓名</tip>
        </div>
    </div>
    ...
    <div class="layui-form-item">
        ...
        <div class="layui-input-block">
            <c:forEach items="${roles}" var="role">
                <input type="checkbox" value="${role.id}" <c:if test="${checkedRoles.contains(role.id)}" >checked=""</c:if> title="${role.roleName}">
            </c:forEach>
        </div>
    </div>
    ...
    <div class="layui-form-item">
        ...
        <div class="layui-input-block">
            <input type="radio" name="locked" value="1" title="可用" <c:if test="${user.locked == 1}" >checked=""</c:if> >
            <input type="radio" name="locked" value="2" title="禁用" <c:if test="${user.locked == 2}" >checked=""</c:if> >
        </div>
    </div>
